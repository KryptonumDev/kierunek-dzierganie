...
const Input = ({ textarea, label, register, errors, password, isRegister }: Props) => {
  const [showPassword, setShowPassword] = useState(false);
  const [value, setValue] = useState('');

  return (
    <label
      className={`${styles['Input']} ${errors[register.name] ? styles['errored'] : ''} ${password ? styles['password'] : ''}`}
    >
