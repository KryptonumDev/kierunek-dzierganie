...
const breadcrumbs = [
  ...
  { name: 'Szydełkowanie', path: '/produkty/szydelkowanie' },
];
...
const query = async (searchParams: { [key: string]: string }): Promise<CrochetingProductsPage_QueryTypes> => {
  return await sanityFetch<CrochetingProductsPage_QueryTypes>({
    ...
    tags: ['CrochetingProducts_Page', 'product', 'productCategory', 'voucher'],
  });
};

export async function generateMetadata() {
  return await QueryMetadata('CrochetingProducts_Page', breadcrumbs[1]!.path);
}
